...
public static class RequestProcess{
    public static string FormProcess(HttpRequest request) {
            ...
            if (_form.Files.Count > 0) {
                ...
                foreach (IFormFile formFile in _form.Files)
                {
                    if (formFile.Length > 0)
                    {
                        ...
                        if (!Directory.Exists("wwwroot/upload/"))  Directory.CreateDirectory("wwwroot/upload/"); // nếu chưa có thì tạo ra folder
        ...
        string format   =  File.ReadAllText("formtest.html");   // Đọc nội dung HTML từ file
